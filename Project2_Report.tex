...
Making the assumption that \(X_I(F)\) does not overlap in frequency with \(X_a(F)\), we can re-define \(X_d(F)\) as
...
First, let us visualize the different components we will be using. Firstly, H(F)
...
We can see that just like the piecewise function describes, H(F) is a box centered about 0, extending from -22 kHz to 22 kHz. Because we know that \(X_a(F)\) must lie in this region.
Assuming that the frequency for \(X_a(F)\) is \(\pm\)22 kHz, no matter the carrier frequency, \(F_c\), the first two terms of \(X_d(F)\) will lie outside of the range of H(F)
